...
```{r message=FALSE, echo=F, out.width='70%', fig.show='hold'}
...
Examining significant correlations among the independent variables, we see that four of the pairs have a correlation close to 1. This can lead to autocorrelation errors in our analysis.
...
train<-train%>%
   ...
   select(-rowsum)
```
We will look at the patterns and intersections of missing data among the variables. We can see that only 22 of the observations have all 5 variables missing, we will exclude these cases. Overall, the pattern suggests that the variables are Missing at Random (MAR)
...
train2 %>% lmmulti(dependent, explanatory) %>% fit2df(metrics=T, condense=T, remove_intercept=F) -> t0
kable(t0[[1]], row.names=FALSE, align=c("l", "l", "r", "r", "r")) %>%
  kable_styling()
kable(t0[[2]], row.names=FALSE, col.names="")%>%
  kable_styling()
```
...
train2 %>%  lmmulti(dependent, explanatory) %>%
  fit2df(metrics=T, condense=T, remove_intercept=F) -> t1
  #finalfit.lm("TARGET_WINS", colnames(train2[,-1]), metrics=TRUE , add_dependent_label=TRUE) -> t1 
kable(t1[[1]], row.names=FALSE, align=c("l", "l", "r", "r", "r")) %>%
  kable_styling()
kable(t1[[2]], row.names=FALSE, col.names="")%>%
  kable_styling()
```
...
train_log %>%  lmmulti(dependent, explanatory) %>%
  fit2df(metrics=T, condense=T, remove_intercept=F) -> t2
  #finalfit.lm("TARGET_WINS", colnames(train2[,-1]), metrics=TRUE , add_dependent_label=TRUE) -> t1 
kable(t2[[1]], row.names=FALSE) %>%
  kable_styling()
kable(t2[[2]], row.names=FALSE, col.names="")%>%
  kable_styling()
```
...
train_bx_transformed %>%  lmmulti(dependent, explanatory) %>%
  fit2df(metrics=T, condense=T, remove_intercept=F) -> t3
  #finalfit.lm("TARGET_WINS", colnames(train2[,-1]), metrics=TRUE , add_dependent_label=TRUE) -> t1 
kable(t3[[1]], row.names=FALSE) %>%
  kable_styling()
kable(t3[[2]], row.names=FALSE, col.names="")%>%
  kable_styling()
```
...
explanatory <- colnames(train_knn_log[,-1])
  #c("BATTING_H","BATTING_2B","BATTING_3B","BATTING_HR","BASERUN_SB","PITCHING_H", "PITCHING_BB", "FIELDING_E", "FIELDING_DP")
#model1 <- lm(TARGET_WINS ~ ., data = subset(train2, select=append(explanatory, dependent)))

train_knn_log %>%  lmmulti(dependent, explanatory) %>%
  fit2df(metrics=T, condense=T, remove_intercept=F)  -> t4
  #finalfit.lm("TARGET_WINS", colnames(train2[,-1]), metrics=TRUE , add_dependent_label=TRUE) -> t1 
kable(t4[[1]], row.names=FALSE) %>%
  kable_styling()
kable(t4[[2]], row.names=FALSE, col.names="")%>%
  kable_styling()
```
...
It seems that model 4's performance is almost consistent with other models in terms of collinearity and RMSE but adjusted r2 is comparatively better than others. The model, also makes intuitive sense. This suggests it should be selected. 
